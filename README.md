...
FANZAの高評価作品を検索し、MissAVで動画検索もできる多機能Discord BOTです。

**🆕 最新アップデート**: 
- **🔄 コマンド名変更** - `fanza_sale` → `fanza_search` でより直感的に！
- **🔍 セールフィルター機能** - デフォルトは全作品検索、セール商品も選択可能！
- **⚡ スクレイピング高速化** - Playwrightの並列処理により最大10倍高速化を実現！
- **強化されたフィルタリング** - ソート順、キーワード検索、配信開始日フィルターを追加！
- **6種類のソート順** - おすすめ順、人気順、売上順、新着順、評価順、お気に入り順
- **キーワード検索** - 作品名、女優名などで検索・絞り込み可能
...
```
**基本機能**:
- デフォルト: 全作品から高評価（4.0+）作品を検索
- セールフィルター: 期間限定、割引、日替わり、激安セールから選択可能
- 表示モード: 評価順、ランダム、リスト形式
...
**変更点**:
- ❌ 旧: `/fanza_sale` → ✅ 新: `/fanza_search`
- セール商品だけでなく、**全FANZA作品**が検索対象に
- セールフィルターは**オプション機能**として利用可能

### 🔍 柔軟な検索オプション
**デフォルト動作**:
- 全作品から高評価（4.0+）作品を検索
- セール制限なし、より多くの作品が見つかります
...
### 📈 改善されたユーザー体験
- **より多くの検索結果**: セール制限がないため、より豊富な作品が見つかります
- **直感的な操作**: 必要に応じてセールフィルターを選択できます
- **柔軟性**: ユーザーのニーズに合わせた検索が可能
...
# 期間限定セール商品のみ
/fanza_search sale_type:期間限定セール

# VR作品のみ検索
/fanza_search media_type:VRのみ
```
...
# 最新作をランダム表示
/fanza_search sort_type:新着順 release_filter:最新作 mode:ランダム count:3

# 割引セールを人気順で表示
/fanza_search sale_type:割引セール sort_type:人気順 count:8

# リスト形式で一覧表示
/fanza_search mode:リスト形式 count:10
```
...
# 激安セールを新着順で
/fanza_search sale_type:激安セール sort_type:新着順
```
